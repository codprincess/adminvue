    ...
    <el-row class="container">
            ...
            <el-col :span="4" class="userinfo">
                <el-dropdown trigger="hover">
                    <span class="el-dropdown-link userinfo-inner">
                        <img :src="this.sysUserAvatar" alt="图片"/>
                        {{sysUserName}}
                    </span>
        ...
        <el-col :span="24" class="main">
            <aside :class="collapsed?'menu-collapsed':'menu-expanded'">
                <!--导航菜单-->
                <el-menu class="el-menu-vertical-demo" @open="handleopen" @close="handleclose" @select="handleselect" unique-opened router v-show="!collapsed">
                    ...
                    <li class="el-submenu item" v-for="(item,index) in $router.options.routes" v-if="!item.hidden">
                        <template v-if="!item.leaf">
                            ...
                            <ul class="el-menu submenu" :class="'submenu-hook-'+index" @mouseover="showMenu(index,true)" @mouseout="showMenu(index,false)">
                                <li class="el-menu-item" style="padding-left:40px;" v-for="child in item.children" v-if="!child.hidden" :key="child.path" :class="$route.path==child.path?'is-active':''" @click="$router.push(child.path)">{{child.name}}</li>
                            </ul>
                        </template>
                        <template v-else>
                            <li class="el-submenu">
                                <div :class="$route.path==item.children[0].path?'is-active':''" @click="$router.push(item.children[0].path)" class="el-submenu__title el-menu-item" style="padding-left:20px;height:56px;line-height:56px;padding:0 20px;">
                                    <i :class="item.iconCls"></i>
                                </div>
                            </li>
                        </template>
